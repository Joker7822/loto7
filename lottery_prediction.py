...
# =========================
# Monkey patch: auto-save self predictions from LotoPredictor.predict
# =========================
try:
    import numpy as _np
except Exception:
...
def __patched_predict(self, latest_data, num_candidates=50):
    # call the original (renamed) implementation
    preds = __orig_predict(self, latest_data, num_candidates=num_candidates)
    try:
        # If original returns just None (in error paths), do nothing
        if preds is None:
            return preds
        # Support both shapes:
        # - (pred_numbers, confidences) or
        # - list[ (numbers, confidence) ]
        to_save = []
        if isinstance(preds, tuple) and len(preds) == 2:
            pred_numbers, confidences = preds
            for nums, conf in zip(pred_numbers, confidences):
                if _np is not None:
                    nums = _np.array(nums)
                to_save.append((nums, float(conf)))
        else:
            for item in preds:
                nums, conf = item
                if _np is not None and not isinstance(nums, list):
                    nums = _np.array(nums)
                to_save.append((nums, float(conf)))
        ...
        if _save_self_predictions is not None and len(to_save) > 0:
            _save_self_predictions(to_save, file_path="self_predictions.csv", max_records=100)
    except Exception as e:
        print(f"[PATCH] save_self_predictions failed: {e}")
    return preds

# Only patch once
if not globals().get("__predict_patched__", False):
    try:
        # Keep original reference
        __orig_predict = LotoPredictor.predict
        # Replace with patched version
        LotoPredictor.predict = __patched_predict
        __predict_patched__ = True
        print("[PATCH] LotoPredictor.predict has been patched to auto-save self predictions.")
    except Exception as e:
        print(f"[PATCH] Failed to patch LotoPredictor.predict: {e}")
